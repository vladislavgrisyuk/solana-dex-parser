...
impl TransactionAdapter {
    ...
    fn extract_token_maps(
        ...
        for balance in &tx.post_token_balances {
            Self::record_token_balance(&mut accounts, &mut decimals, balance);
        }
    ...
    fn record_token_balance(
        ...
        accounts
            ...
            .or_insert_with(|| Self::token_info_from_balance(balance));
        decimals
    ...
    fn token_info_from_balance(balance: &TokenBalance) -> TokenInfo {
        ...
        TokenInfo {
            ...
            authority: balance.owner.clone(),
            destination: Some(balance.account.clone()),
            destination_owner: balance.owner.clone(),
            source: Some(balance.account.clone()),
            ..TokenInfo::default()
        }
    }
}
